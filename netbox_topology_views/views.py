...
def create_edge(
    ...
    edge["cable_a_dev_name"] = cable_a_dev_name
    edge["cable_b_dev_name"] = cable_b_dev_name
    edge["cable_a_speed"] = termination_a.get("speed")
    edge["cable_b_speed"] = termination_b.get("speed")
    title = "Cable"
...
def get_topology_data(
        ...
        for interface in interfaces:
            ...
            for destination in interface._path.destinations:
                if isinstance(destination, device_components.Interface):
                    ...
                    edge_ids += 1
                    termination_a = { "termination_name": interface.name, "termination_device_name": interface.device.name, "device_id": interface.device.id, "speed": getattr(interface, 'speed', None) }
                    termination_b = { "termination_name": destination.name, "termination_device_name": destination.device.name, "device_id": destination.device.id, "speed": getattr(destination, 'speed', None) }
    ...
    if show_cables:
        ...
        ).select_related("termination_type", "cable")
        ...
        for link in links:
            ...
            if link.termination_type.model in supported_termination_types:
                if link.cable_id in processed_cables:
                    continue
                processed_cables.add(link.cable_id)
                ...
                for idx in range(min(len(a_terms), len(b_terms))):
                    term_a_obj = a_terms[idx]
                    term_b_obj = b_terms[idx]

                    if hasattr(term_a_obj, "device") and term_a_obj.device is not None:
                        if term_a_obj.device.pk not in nodes_devices:
                            nodes_devices[term_a_obj.device.pk] = term_a_obj.device
                    
                    if hasattr(term_b_obj, "device") and term_b_obj.device is not None:
                        if term_b_obj.device.pk not in nodes_devices:
                            nodes_devices[term_b_obj.device.pk] = term_b_obj.device

                    termination_a = {
                        "termination_name": term_a_obj.name,
                        "termination_device_name": term_a_obj.device.name if hasattr(term_a_obj, "device") and term_a_obj.device else "Unknown",
                        "device_id": term_a_obj.device.pk if hasattr(term_a_obj, "device") and term_a_obj.device else None,
                        "speed": getattr(term_a_obj, 'speed', None),
                    }

                    termination_b = {
                        "termination_name": term_b_obj.name,
                        "termination_device_name": term_b_obj.device.name if hasattr(term_b_obj, "device") and term_b_obj.device else "Unknown",
                        "device_id": term_b_obj.device.pk if hasattr(term_b_obj, "device") and term_b_obj.device else None,
                        "speed": getattr(term_b_obj, 'speed', None),
                    }
